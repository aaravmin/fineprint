...
export function buildBinderExport(inputs: BinderInputs): BinderExport {
  ...
  return {
    ...
    building_summary: {
      ...building,
      source_note:
        "Building facts from NYC public records (PLUTO, energy benchmarking disclosure, DOB) where available.",
    },
    compliance_snapshot: {
...
// Article 321 rides inside the LL97 obligation, so it gets no separate row.
export function binderLawOrder() {
  return lawsInOrder().filter((law) => law.law_id !== "art321");
}
